<footer class="page-footer indigo darken-2">
    <div class="container">
        <div class="row">
            <div class="col m4 s12">
                <h5 class="white-text uppercase">О Нас</h5>
                @if(isset($footersettings[0]) && $footersettings[0]['aboutus'])
                    <p class="grey-text text-lighten-4">{{ $footersettings[0]['aboutus'] }}</p>
                @else
                    <p class="grey-text text-lighten-4">АН "Донбасс" — агентство недвижимости, специализирующееся на продаже, аренде и подборе жилья в регионе. Мы предлагаем профессиональный подход, индивидуальные решения и полное сопровождение сделок.</p>
                @endif
            </div>
                ...
                <ul class="collection border0">
                    ...
                    <li class="collection-item transparent clearfix p-0 border0">
                        ...
                        <div class="float-left">
                            ...
                            <p class="m-t-0 m-b-5 grey-text text-lighten-1">Спальня: {{ $property->bedroom }} Ванная: {{ $property->bathroom }} </p>
                        </div>
                    </li>
                    @endforeach
            ...
            <div class="col m2 s12">
                <h5 class="white-text uppercase">Меню</h5>
                <ul>
                    <li class="uppercase {{ Request::is('property*') ? 'underline' : '' }}">
                        <a href="{{ route('property') }}" class="grey-text text-lighten-3">Недвижимость</a>
                    </li>

                    <li class="uppercase {{ Request::is('agents*') ? 'underline' : '' }}">
                        <a href="{{ route('agents') }}" class="grey-text text-lighten-3">Агенты</a>
                    </li>

                    <li class="uppercase {{ Request::is('gallery*') ? 'underline' : '' }}">
                        <a href="{{ route('gallery') }}" class="grey-text text-lighten-3">Галерея</a>
                    </li>

                    <li class="uppercase {{ Request::is('blog*') ? 'underline' : '' }}">
                        <a href="{{ route('blog') }}" class="grey-text text-lighten-3">Новости</a>
                    </li>

                    <li class="uppercase {{ Request::is('contact') ? 'underline' : '' }}">
                        <a href="{{ route('contact') }}" class="grey-text text-lighten-3">Наши контакты</a>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="footer-copyright">
        <div class="container">
            ...
            @else
                © 2024 Разработано для ВКР Прачёв Д. И.
            @endif

                @if(isset($footersettings[0]) && $footersettings[0]['vkontakte'])
                    <a class="grey-text text-lighten-4 right" href="{{ $footersettings[0]['facebook'] }}" target="_blank">Вконтакте</a>
                @endif
            @if(isset($footersettings[0]) && $footersettings[0]['facebook'])
                <a class="grey-text text-lighten-4 right" href="{{ $footersettings[0]['facebook'] }}" target="_blank">Лицеваякнига</a>
            @endif
            @if(isset($footersettings[0]) && $footersettings[0]['twitter'])
                <a class="grey-text text-lighten-4 right m-r-10" href="{{ $footersettings[0]['twitter'] }}" target="_blank">Х-виттер</a>
            @endif
